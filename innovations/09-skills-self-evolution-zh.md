...
> **上一篇**：[08 两层面范围蔓延的分治策略](08-scope-creep-bifurcation-zh.md)
> **下一篇**：[10 7+1 意图→策略自动切换](10-intent-routing-zh.md)

---

> **摘要**：Agent 每完成一次复杂任务都消耗了大量推理 Token——但如果下次遇到类似任务，那些 Token 不会自动复用。本文提出 Skills 自进化闭环：Agent 成功完成任务后，自动将可复用的执行路径固化为 Skill；下次遇到同类场景时 Skill 自动加载，直接复用而不需要重新推理。这是 Token 节省金字塔的第一层（Skills 固化）——Hermes 是目前唯一实现该机制的 Agent 平台。本质上，这是人的「技能点化」过程在 Agent 身上的映射：重复做的事情，就应该变成肌肉记忆。
...
- **Token 节省效果**：以「写一篇公众号文章」为例，固化后的 `article-workflow-v2` Skill 将任务启动的推理 Token 从 ~3K 降至 ~200（Skill 列表中的一条描述）。任务执行中的步骤选择不再需要从头推理——按 Skill 清单执行。
...
Hermes 是目前唯一实现了 Skills 自进化闭环的 Agent 平台。这是它在 8 款产品对比中「少数同时具备的四项能力」之一（Memory + Skills 自进化 + Cronjob + Gateway）。
...
*上一篇：[08 两层面范围蔓延的分治策略](08-scope-creep-bifurcation-zh.md) — 为什么要区分需求蔓延和技术蔓延。下一篇：[10 7+1 意图→策略自动切换](10-intent-strategy-switching-zh.md) — 不同任务意图，Agent 应该自动切换不同的 Planning 策略。*
