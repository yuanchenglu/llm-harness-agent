...
|  | [QA(“Who is the publisher of The New |  |  |
| --- | --- | --- | --- |
| England Journal of Medicine?”) |  | → | Massachusetts |
| Medical Society] |  |  |  |
...
sample   a   position   i   and   corresponding   API   call   candidates   c 1
i   , c 2
i   , . . . , c k
i   .   We   then   execute   these   API   calls   and
filter   out   all   calls   which   do   not   reduce   the   loss   L i   over   the   next   tokens.   All   remaining   API   calls   are   interleaved
...
each such call.
We represent each API call as a tuple  c  = ( a c , i c )
where  a c   is the name of the API and  i c   is the cor-
...
Given   a   dataset   C   =   { x 1 , . . . ,  x |C| }   of   plain
...
1 In practice,   we use the token sequences “   [ ”,   “ ] ” and
...
| LM Dataset | 1 2 3 Sample API Calls Execute API Calls Filter API Calls c1 = What other name is r1 = Steel City L(c1 → Steel City) i i i i Pittsburgh known by? < min(L(c1 → ε), L(ε)) i i i c2 = Which country is r2 = United States L(c2 → United States) i i i i Pittsburgh in? > min(L(c2 → ε), L(ε)) i i i | LM Dataset with API Calls x* = Pittsburgh is also known as [QA(What …? → Steel City)] the Steel City. |  |
| --- | --- | --- | --- |
| x = Pittsburgh is 1:i-1 also known as x = the Steel City i:n | c1 = What other name is i Pittsburgh known by? c2 = Which country is i Pittsburgh in? |  | x* = Pittsburgh is also known as [QA(What …? → Steel City)] the Steel City. |
|  |  |  |  |
...
| (c1 → i i | Steel City |  |  | ) |  |
| --- | --- | --- | --- | --- | --- |
| min( | L(c1 i i | → | ε), | L( i | ε)) |
| (c2 → United States) i i |  |  |  |  |  |
| (c2 → i i | United States |  |  |  |  |
| min( | L(c2 i i | → | ε) | , L( i | ε)) |
...
c i  needs to be a single text sequence  r i .
Filtering API Calls
Let  i  be the position of the
API call  c i  in the sequence  x  =  x 1 , . . . , x n , and let
r i   be the response from the API. Further, given a
sequence  ( w i   |  i  ∈ N )  of  weights , let
L i ( z ) =  −
n
�
j = i
w j − i  ·  log  p M ( x j   |  z , x 1: j − 1 )
be the weighted cross entropy loss for  M   over the
tokens  x i , . . . , x n   if the model is prefixed with  z .
We compare two different instantiations of this loss:
L +
i   =  L i ( e ( c i , r i ))
L −
i   = min ( L i ( ε ) , L i ( e ( c i , ε )))
where  ε  denotes an empty sequence.   The former is
the weighted loss over all tokens  x i , . . . , x n  if the
...
its   input.   Given   a   filtering   threshold   τ f ,   we   thus
only keep API calls for which
L −
i   − L +
i   ≥ τ f
holds, i.e., adding the API call and its result  reduces
the loss by at least  τ f , compared to not doing any
API call or obtaining no result from it.
Model Finetuning
After sampling and filtering
calls for all APIs, we finally merge the remaining
API   calls   and   interleave   them   with   the   original
inputs.   That is,   for an input text  x   =   x 1 , . . . , x n
with a corresponding API call and result  ( c i , r i )  at
position   i ,   we   construct   the   new   sequence   x ∗ =
3 We provide  e ( c i , r i )  as a prefix instead of inserting it at
...
x 1: i − 1 ,  e ( c i , r i ) , x i : n ; we proceed analogously for
...
Inference
When   generating   text   with   M   after
...
et al. ,  2019 ).
Calculator
As a second tool, we use a calculator
...
Text  classifier ( Joulin et al. ,  2016 ), while the target
language is always set to English.
Calendar
Our   final   tool   is   a   calendar   API   that,
...
different values of our filtering threshold  τ f .
...
els:
4 While this filtering alters the distribution of training exam-
...
5 This is achieved by manually setting the probability of
the  <API>  token to 0.
6 We use the original  davinci  variant that is not finetuned
on any instructions.
...
7 An exception to this is if the model’s prediction contains
...
uate all models on   T EMP LAMA   ( Dhingra et al. ,
2022 )   and   a   new   dataset   that   we   call   D ATESET .
...
tween   2010   and   2020.   D ATESET ,   described   in
...
former   outperforms   all   baselines   for   both   T EM -
P LAMA and D ATESET .   However, closer inspec-
tion   shows   that   improvements   on   T EMP LAMA
...
For D ATESET , on the other hand, the consider-
...
8 We   do   not   evaluate   the   perplexity   of   Toolformer   with
API   calls   enabled   as   computing   the   probability   p M ( x t   |
x 1 , . . . , x t − 1 )  of token  x t   given  x 1 , . . . , x t − 1   would require
...
Tool Use
Several approaches aim to equip LMs
...
Bootstrapping
The   idea   of   using   self-training
...
moyer.   2021.
Htlm:   Hyper-text   pre-training   and
...
we   use   values   of   τ s   =   0 . 05   and   τ f   =   1 . 0   –   i.e.,
...
we   set   τ s   =   0 . 0 ,   k   =   20   and   m   =   10   for   these
tools.   As   the   resulting   sets   of   API   calls   are   still
comparably small, we additionally set  τ f   = 0 . 5 .
A.1
Implementation
Question Answering
...
Calendar
For creating our dataset  C ∗ , we operate
...
tected   using   the   fastText   classifier   ( Joulin   et   al. ,
...
Output:
Calculator
We use the following prompt for the
calculator:
...
区
设
计 a   plane   of   reference   Gaochun   is
one   of   seven   districts   of   the   provincial
capital   Nanjing
Output:   [MT( 南 京 高 淳 县 住 房 和 城 乡 建 设 局 城 市 新
区 设计 )]   a   plane   of   reference   Gaochun   is
one   of   seven   districts   of   the   provincial
capital   Nanjing
Input:   x
Output:
...
D ATESET ,   we   simply   prefix   the   question   with
...
D
D ATESET
D ATESET  is created by first randomly selecting 500
