...
2. **OMO（Oh My OpenAgent）插件**
   社区驱动插件。7+1 意图分类（重构/新建/中等/协作/架构/调研/简单 + Spec-Driven）提供了值得研究的意图→策略切换方案。
...
### CodeWhale（围绕 DeepSeek 模型构建的社区开源项目）

**我把它当什么用**：研究 DeepSeek 模型在 Agent 场景下实际表现的实验平台

**背景**：CodeWhale 是围绕 DeepSeek 模型构建的社区开源 Coding Agent（曾使用 DeepSeek-TUI 名称）；它不是 DeepSeek 官方产品。我没有参与开发，但深度使用它来验证 DeepSeek 模型在 Agent 架构下的真实表现——因为它重点适配 DeepSeek API，适合观察 DeepSeek-specific provider、cache 与 reasoning 策略。
