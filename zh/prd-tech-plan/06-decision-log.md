...
## 9. 2026-06-18 中文 B 端桌面定位决策

决策：Desktop 默认产品定位从 consumer personal assistant / Code-first workbench 改为中文 B 端本地任务客户端，目标用户是财务、行政、产品经理和律师。

结果：

- 首屏默认简体中文。
- 默认信息架构改为项目、任务列表、技能、自动化、设置。
- Kun/Quinn 的侧栏、中央工作区、底部输入框和轻视觉语言作为硬参考。
- `Code`、diff、route、cache、cost、raw evidence、diagnostics 改为高级入口或任务抽屉内容。
- 手机连接、插件市场、团队协作、云端自动化、Excel/PDF/OCR/合同深度解析不进入本轮首屏能力。

理由：

- 当前 UI 容易被财务和行政用户理解为运维/工程诊断工具。
- 商业化产品必须先解决普通业务人员能否学会、用懂、用明白。
- 现有 bridge 已能支撑首轮前端重组，无需扩张后端能力。
- 未验证能力若提前展示，会制造错误承诺。
