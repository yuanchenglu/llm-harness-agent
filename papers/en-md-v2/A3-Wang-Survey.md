...
Zhi-Yuan Chen, Jiakai Tang, Xu Chen( � ), Yankai Lin( � ), Wayne Xin Zhao,
...
Keywords
Autonomous   agent,   Large   language
...
as to e ff ect what it senses in the future.”
...
ChatDev 2023-7
Fig. 1
Illustration of the growth trend in the field of LLM-based autonomous agents.   We present the cumulative number of
papers published from January 2021 to August 2023.   We assign di ff erent colors to represent various agent categories.   For
...
di ff ers from the human learning process, since the
...
Furthermore, LLM-based agents can o ff er natural
...
plete   various   tasks   e ff ectively.   Previously,   these
models were proposed independently, with limited
e ff orts made to summarize and compare them holis-
...
ity to complete di ff erent tasks.   Intuitively, the first
...
2
LLM-based Autonomous Agent Con-
struction
...
would like to design agents with di ff erent person-
...
ally prompting LLMs with di ff erent roles, such as
...
time and e ff ort required to construct agent profiles,
...
consistent, reasonable, and e ff ective manner.   This
...
ories.   The short-term memory temporarily bu ff ers
...
cilitating   e ffi cient   storage   and   retrieval.   Specifi-
...
short-term memory to store di ff erent objects, which
can then be accessed throughout various conversa-
tions.   Users can create multiple conversations with
di ff erent agents on the same canvas, facilitating the
...
memory or embedding memory.   Di ff erent memory
...
enhances both retrieval and reading e ffi ciency.   For
instance, MemoryBank [ 39 ] encodes each memory
segment   as   an   embedding   vector   and   employs   a
dual-tower   dense   retrieval   model   to   e ffi ciently   re-
...
to manipulate memories e ffi ciently and comprehen-
...
memory can be conveyed in an e ffi cient and concise
...
allows for e ffi cient retrieval of memory records.   By
...
m ∈ M
�
α s rec ( q ,  m )  +  β s rel ( q ,  m )  +  γ s imp ( m )
�
,
(1)
...
s rec ( · ),  s rel ( · ) and  s imp ( · ) are the scoring functions for
...
s rel ( q ,  m ) can be calculated using vector similarity
measures between query and memory embeddings.
It should be noted that   s imp   only reflects the charac-
ters of the memory itself, thus it is unrelated to the
query  q .   α ,  β  and  γ  are balancing parameters.   By
assigning them with di ff erent values, one can obtain
various memory reading strategies.   For example, by
setting  α  =  γ  =  0, many studies [ 16 , 30 , 38 , 41 ] only
consider the relevance score   s rel   for memory read-
...
agent to act more e ffi ciently and rationally.   During
...
it is full, people design di ff erent methods to delete
...
RET-LLM [ 41 ] uses a fixed-size bu ff er for memory,
...
Step-3
Fig. 3
Comparison between the strategies of single-path and multi-path reasoning.   LMZSP is the model proposed in [ 44 ].
2.1.3
Planning Module
...
on   Huggingface.   Di ff erent   from   CoT   and   Zero-
...
late the potential benefits of di ff erent plans based
...
power of LLMs in zero-shot planning, e ff ectively
...
are well-developed and employ e ffi cient search algo-
...
an   external   planner,   which   e ffi ciently   determines
...
ternal low-level planner is employed to e ff ectively
...
12
Front.   Comput.   Sci., 2025, 0(0):   1–42
can be less e ff ective due to the following reasons:
firstly, generating a flawless plan directly from the
beginning is extremely di ffi cult as it needs to con-
...
them to more e ff ectively revise their plans.   LLM-
...
feedback   is   a   subjective   signal.   It   can   e ff ectively
...
the above cases, we can see, di ff erent types of feed-
...
vide feedback on the output and o ff er guidance on
...
powerful and capable of e ff ectively addressing com-
...
Action Production : Di ff erent from ordinary LLMs,
where   the   model   input   and   output   are   directly   as-
sociated,   the   agent   may   take   actions   via   di ff erent
...
MetaGPT [ 23 ], di ff erent agents may communicate
...
•   External   Tools .
While   LLMs   have   been
demonstrated   to   be   e ff ective   in   accomplishing   a
...
e ff ectively   mitigating   hallucination   issues   during
...
tool APIs for training.   API-Bank [ 70 ] o ff ers a com-
...
and e ff ectively.   (1)  Planning Capability .   Previous
...
example, in ChatDev [ 18 ], di ff erent agents can dis-
...
relying   solely   on   the   hardware   is   insu ffi cient   for
achieving   e ff ective   task   performance.   This   is   be-
...
17
Table 1
For the profile module, we use  ① ,  ② and  ③ to represent the handcrafting method, LLM-generation method, and
...
respectively.   For the agent  c apability  a cquisition (CA) strategy, we use  ① and  ② to represent the methods with and without
...
human values and preferences.   Di ff erent from the
...
the e ff ectiveness of real-world datasets for domain-
...
back di ff er, the critic generates failure information,
...
with, di ff erent agents provide separate responses to
...
tions with the environment, enabling e ffi cient task
...
be a ff ected by LLM-based autonomous agents.
...
authors   assign   LLMs   with   di ff erent   profiles,   and
...
Evaluation
Fig. 5
The applications (left) and evaluation strategies (right) of LLM-based agents.
“hyper-accuracy distortion”), which may influence
the downstream applications.   In [ 103 ], the authors
systematically   analyze   the   e ff ectiveness   of   LLM-
...
of di ff erent behavioral characteristics of LLM-based
...
S 3   [ 78 ] builds a social network simulator, focusing
...
LLM-based   agents   o ff er   multifaceted   assistance,
...
of LLM-based agents in enhancing the e ffi ciency,
...
the agent can e ffi ciently query and utilize internet
...
programming.   It o ff ers many useful features, such
...
to open-ended questions and o ff er feedback.
3.3
Engineering
...
gineering,   LLM-based   agents   o ff er   potential   for
...
work demonstrates e ffi cient and cost-e ff ective gener-
...
code vulnerabilities.   This approach e ff ectively man-
ages the trade-o ff  between accuracy and scalability.
...
tool based on LLMs, which can e ff ectively identify
...
25
Table 2
Representative applications of LLM-based autonomous agents.
...
at di ff erent levels.   This research demonstrates the
...
more   e ffi cient   reinforcement   learning   agents   for
...
|  | Social Simulation | Social Simulacra [80], Generative Agents [20], SocialAI School [108], AgentSims [34], S3 [78], Williams et al. [109], Li et al. [106], Chao et al. [107] |
...
|  | Natural Science Education | ChemCrow [76], CodeHelp [119], Boiko et al. [114], MathA- gent [116], Drori et al. [117], EduChat [87], FreeText [120] |
...
38 , 79 , 132 – 135 , 137 – 140 ].   These e ff orts focus on
...
LangChain enables e ffi cient and cost-e ff ective soft-
...
velopment tasks.   SmolModels [ 126 ] o ff ers a family
of   compact   language   models   suitable   for   various
tasks.   AGiXT   [ 144 ]   is   a   dynamic   AI   automation
platform that e ffi ciently manages instructions and
...
e ffi ciently.   GPT Researcher [ 150 ] is an experimen-
tal   application   that   leverages   LLMs   to   e ffi ciently
...
o ff ers   a   simple   interface   for   loading   plugins   via
...
4
LLM-based Autonomous Agent Eval-
uation
Similar to LLMs themselves, evaluating the e ff ec-
...
tates   that   human   evaluators   di ff erentiate   between
...
costs, ine ffi ciency, and population bias.   To address
...
resentative   evaluation   metrics.   (1)   Task   success
...
(3)  E ffi ciency metrics:   In contrast to the aforemen-
tioned metrics used to evaluate the agent e ff ective-
ness, these metrics aim to assess the e ffi ciency of
agent.   Commonly considered metrics encompass
the cost associated with development [ 18 ] and train-
ing e ffi ciency [ 16 , 38 ].
...
29
Table 3
For subjective evaluation, we use  ① and  ② to represent human annotation and the Turing test, respectively.   For
...
use a set of diverse tasks from di ff erent domains to
evaluate the agent, which can e ff ectively measure
...
bug detection rate to measure the e ff ectiveness of
LLM-based agents.
...
the e ff ectiveness of di ff erent agent modules in inter-
...
ities of LLM-based agents. WebArena [ 173 ] o ff ers a
...
reasoning,   safety,   and   e ffi ciency,   when   utilizing
...
fers a structured evaluation across varying di ffi culty
...
Di ff erent from traditional LLMs, autonomous agent
usually has to play as specific roles ( e.g. , program
coder,   researcher and chemist) for accomplishing
di ff erent   tasks.   Thus,   the   capability   of   the   agent
for role-playing is very important.   Although LLMs
can e ff ectively simulate many common roles such
...
that is, for di ff erent purposes and applications, the
...
eration and e ff ective communication.   Previous re-
search [ 183 , 184 ] has highlighted the lack of robust-
ness in prompts for LLMs, as even minor alterations
can yield substantially di ff erent outcomes.   This is-
...
e ff ectiveness   of   simulations.   For   instance,   when
...
on.   Consequently, the e ffi ciency of agent actions is
greatly a ff ected by the speed of LLM inference.
7
Conclusion
...
F, others .   Llama:   Open and e ffi cient foundation lan-
...
24.   Dong   Y,   Jiang   X,   Jin   Z,   Li   G.
Self-collaboration
code   generation   via   chatgpt.
arXiv   preprint
arXiv:2304.07590, 2023
25.
Safdari M, Serapio-García G, Crepy C, Fitz S, Romero
...
A,   Narasimhan   K.
Toxicity   in   chatgpt:   Analyzing
persona-assigned   language   models.
arXiv   preprint
...
ing.   2023
32.
Zhu A, Martin L, Head A, Callison-Burch C.   Calypso:
...
2023
35.
Liang X, Wang B, Huang H, Wu S, Wu P, Lu L, Ma Z,
...
37.
Huang Z, Gutierrez S, Kamana H, MacNeil S.   Memory
...
40.
Hu C, Fu J, Du C, Luo S, Zhao J, Zhao H. Chatdb: Aug-
...
24824–24837
46.
Kojima T, Gu S S, Reid M, Matsuo Y, Iwasawa Y. Large
...
52.   Yao   S,   Yu   D,   Zhao   J,   Shafran   I,   Gri ffi ths   T,   Cao   Y,
...
of   ideas   in   large   language   models.
arXiv   preprint
...
61.
Song C H, Wu J, Washington C, Sadler B M, Chao W L,
...
75.
Surís D, Menon S, Vondrick C.   Vipergpt:   Visual infer-
...
robotic a ff ordances.   arXiv preprint arXiv:2204.01691,
2022
...
83.
Liu R, Yang R, Jia C, Zhang G, Zhou D, Dai A M, Yang
...
87.
Dan Y, Lei Z, Gu Y, Li Y, Yin J, Lin J, Ye L, Tie Z, Zhou
...
92.   Mandi   Z,   Jain   S,   Song   S.
Roco:   Dialectic   multi-
robot collaboration with large language models.   arXiv
preprint arXiv:2307.04738, 2023
93.
Zhang C, Liu L, Wang J, Wang C, Sun X, Wang H, Cai
...
96.   Madaan   A,   Tandon   N,   Clark   P,   Yang   Y.
Memory-
...
99.
Saha S, Hase P, Bansal M.   Can language models teach
...
101.
Aher G V, Arriaga R I, Kalai A T. Using large language
...
search, 2023
106.
Li S, Yang J, Zhao K. Are you in a masquerade? explor-
...
119.
Li ffi ton M, Sheese B E, Savelka J, Denny P.   Codehelp:
...
128.
al. e A O.   GPT engineer.   https://github.com/
...
131.
Zhang C, Yang K, Hu S, Wang Z, Li G, Sun Y, Zhang C,
...
132.
Hu B, Zhao C, others . Enabling intelligent interactions
...
136.
Wu J, Antonova R, Kan A, Lepert M, Zeng A, Song S,
...
2023
140.
Zhou W, Peng X, Riedl M.   Dialogue shaping:   Empow-
...
yoheinakajima , 2023
147.
Chase H. langchain.  https://docs.langchain.
com/docs/ , 2023
...
with   agents   through   help   feedback.
arXiv   preprint
...
2023
166.
Lin J, Tomlin N, Andreas J, Eisner J. Decision-oriented
dialogue   for   human-ai   collaboration.   arXiv   preprint
arXiv:2305.20076, 2023
167.
Feldt R, Kang S, Yoon J, Yoo S.   Towards autonomous
...
2023
176.
Yang J, Jin H, Tang R, Han X, Feng Q, Jiang H, Zhong
S,   Yin   B,   Hu   X.
...
180.
Chang Y, Wang X, Wang J, Wu Y, Yang L, Zhu K, Chen
...
183.   Zhuo T Y, Li Z, Huang Y, Shiri F, Wang W, Ha ff ari G,
